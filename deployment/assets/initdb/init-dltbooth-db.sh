...
psql -v ON_ERROR_STOP=1 --username $POSTGRES_USER --dbname $POSTGRES_DB <<-EOSQL
  CREATE USER $SED_PERSISTENCE_DLTBOOTH_USER WITH ENCRYPTED PASSWORD '$SED_PERSISTENCE_DLTBOOTH_PASSWORD';
  CREATE DATABASE $SED_PERSISTENCE_DLTBOOTH_DATABASE;
  GRANT ALL PRIVILEGES ON DATABASE $SED_PERSISTENCE_DLTBOOTH_DATABASE TO $SED_PERSISTENCE_DLTBOOTH_USER;
  \c $SED_PERSISTENCE_DLTBOOTH_DATABASE $SED_PERSISTENCE_DLTBOOTH_USER
